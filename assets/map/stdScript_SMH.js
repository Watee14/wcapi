...
function renderInspection() {
	...
	$.ajax({
		...
		$.each(obj , function(key, value) {
			...
			//console.log(value['inspection_area_code']);
			 //console.log(latlon);
			var hex = hexToRgb(value['bg_color']);
			var geom2 = new longdo.Polygon(latlon_2, {
				title: value['police_station_code']+"-"+value['inspection_area_code'] ,
				// title: '' ,
				detail: '',
				//label: value['inspection_area_code'],
				lineWidth: 0.7,
				weight: 1,
				lineWidth: 0,
  				lineColor: 'rgba(0, 0, 0, 0.0)', // ซ่อนเส้นขอบ
  				fillColor: 'rgba(0, 0, 0, 0.0)' // ซ่อนสี polygon*/
...
function clearMarker() {
	...
	$.each(mark, function(key, value) {
		...
		if(value instanceof longdo.Marker == true ){
			...
			if(value.type != 'police_car' && value.type != 'citizen'){
