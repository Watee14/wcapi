...
class Config_model extends CI_Model
{
	...
	public function readConfig( $table )
	{
		...
		foreach ($config as $key => $value)
		{
			...
			else if ($table == 'case_inform')
			{
				$ds[$value["inform_id"]] = $value;
			}
			else if ($table == 'case_closejob')
			{
				$ds[$value["closejob_code"]] = $value;
			}
			else if ($table == 'map_type')
			{
	...
	public function readSystemSetting()
	{
		$results = array();
		foreach ($this->readConfigStd('system_setting') as $keys => $values)
		{
			$results[$keys] = array();
			foreach ($values as $key => $value)
			{
				$results[$keys][$key] = $value['value'];
			}
		}
		return $results;
	}
