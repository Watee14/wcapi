...
class Twiggy
{
	...
	*/
	...
	public function __construct()
	{
		...
		$this->_twig->addGlobal("session",	isset($_SESSION) ? $_SESSION : NULL);

		// print_r($_SESSION);
		// $this->_twig->addGlobal('session', $_SESSION);
		// $this->_globals['session'] = $_SESSION;

		$this->_twig->addGlobal('users',	isset($_SESSION['user']) ? $_SESSION['user'] : NULL);

		$this->_twig->addGlobal('_IS_LABMY', 	defined('_IS_LABMY') 	? _IS_LABMY	: NULL);
		$this->_twig->addGlobal('_IS_LABSQ', 	defined('_IS_LABSQ') 	? _IS_LABSQ	: NULL);
		$this->_twig->addGlobal('_IS_LOCAL', 	defined('_IS_LOCAL') 	? _IS_LOCAL	: NULL);
		$this->_twig->addGlobal('_KB_URL', 		defined('_KB_URL') 		? _KB_URL 	: NULL);
		$this->_twig->addGlobal('_SECURE',		defined('_SECURE') 		? _SECURE 	: NULL);

		$this->_twig->addGlobal('FORM_VALIDATION_INTEGER', 		defined('FORM_VALIDATION_INTEGER') 		? unserialize(FORM_VALIDATION_INTEGER) 		: NULL);
		$this->_twig->addGlobal('FORM_VALIDATION_IS_UNIQUE', 	defined('FORM_VALIDATION_IS_UNIQUE') 	? unserialize(FORM_VALIDATION_IS_UNIQUE) 	: NULL);
		$this->_twig->addGlobal('FORM_VALIDATION_REQUIRED', 	defined('FORM_VALIDATION_REQUIRED') 	? unserialize(FORM_VALIDATION_REQUIRED) 	: NULL);
		$this->_twig->addGlobal('MAPCONF', 						defined('MAPCONF') 						? unserialize(MAPCONF) 						: NULL);
		$this->_twig->addGlobal('TITLE_MODE', 					defined('TITLE_MODE') 					? unserialize(TITLE_MODE) 					: NULL);

		$this->_twig->addGlobal('DATATABLES_LANGUAGE',	defined('DATATABLES_LANGUAGE')	? json_encode(unserialize(DATATABLES_LANGUAGE))	: NULL);

		$this->_twig->addGlobal('MAPTYPE', 			defined('MAPTYPE') 			? MAPTYPE 			: NULL);
		$this->_twig->addGlobal('VERSIONING', 		defined('VERSIONING') 		? VERSIONING 		: NULL);
		$this->_twig->addGlobal('VERSIONALT', 		defined('VERSIONALT') 		? VERSIONALT 		: NULL);
		$this->_twig->addGlobal('DEFAULT_CASE_ID', 	defined('DEFAULT_CASE_ID') 	? DEFAULT_CASE_ID 	: NULL);
		$this->_twig->addGlobal('THAI_DATE', 		defined('THAI_DATE') 		? THAI_DATE 		: NULL);
		$this->_twig->addGlobal('THAI_MONTHYEAR', 	defined('THAI_MONTHYEAR') 	? THAI_MONTHYEAR 	: NULL);
		$this->_twig->addGlobal('THAI_DATETIME', 	defined('THAI_DATETIME') 	? THAI_DATETIME 	: NULL);

		$this->_twig->addGlobal('_MAP_SERVER', 	defined('_MAP_SERVER') 	? _MAP_SERVER 	: NULL);
		$this->_twig->addGlobal('_MAP_KEY', 	defined('_MAP_KEY') 	? _MAP_KEY 	: NULL);

		$this->_twig->addGlobal('_LOGS', 	defined('_LOGS') 	? _MAP_KEY 	: NULL);
		...
		$FONT_SIZING = '';
		if (isset($_GET['fs']) && ! empty($_GET['fs']))
		{
			$FONT_SIZING = '<style type="text/css">*,.accordion-toggle:after,.nav,.nav *,.select2-container--bootstrap .select2-results__option,.select2-search--dropdown .select2-search__field{font-size: '.$_GET['fs'].'px!important}.select2-container--bootstrap .select2-selection--single,.select2-container--bootstrap .select2-selection--multiple{height:'.($_GET['fs'] + 28).'px!important;padding:10px '.($_GET['fs'] - 2).'px!important</style>';
		}
		$this->_twig->addGlobal('FONT_SIZING', $FONT_SIZING);
	}
