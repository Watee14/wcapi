...
class PantipInbox extends CI_Controller {
	...
	public function getInbox( $pantipID="" , $agentID="" )
	{
		//$url = 'https://pantip.com/message/'.$pantipID ;
 
		//$res = $this->readInbox($url, _PANTIP_USER , _PANTIP_PASS );

		$time_start = microtime(true);

		$url = pm_get_message_by_id.'?msgid='.$pantipID.'&access_token='._PANTIP_TOKEN;
		$res = $this->curl_req($url, 'GET' , array() );
		$resData = json_decode($res , true) ;
		if( isset($resData['error']) ){
			$this->Writelogs_model->wLogs( $agentID , $_SERVER['REMOTE_ADDR'] , 'ERROR', 'PT-GetInbox' ,  "" , $time_start , '-1', $pantipID , $res , 'Social'  ) ;
		}else{
			$this->Writelogs_model->wLogs( $agentID , $_SERVER['REMOTE_ADDR'] , 'INFO', 'PT-GetInbox' , "" , $time_start , '0', $pantipID  ,  $res , 'Social'  ) ;
		}
	...
	function sendMessage(){
		...
		if(isset($req['chat_id']) && $req['chat_id'] !='' ){
			...
			$this->Writelogs_model->wLogs( $req['agentID']  , $_SERVER['REMOTE_ADDR'] , 'Error', 'PT-SentInbox' , 'ไม่มีรหัสกระทู้', $time_start , '-1', json_encode( $req) , json_encode($ret), 'Social' ) ;
		...
		if(isset($req['message']) && $req['message'] !='' ){
			...
			$this->Writelogs_model->wLogs( $req['agentID']  , $_SERVER['REMOTE_ADDR'] , 'Error', 'PT-SentInbox' , 'ไม่มีข้อความ', $time_start , '-1', json_encode( $req) , json_encode($ret), 'Social' ) ;
			print_r( json_encode($ret)) ; die();
		}
		...
		$this->Writelogs_model->wLogs( $req['agentID']  , $_SERVER['REMOTE_ADDR'] , 'Success', 'PT-SentInbox' , 'CheckResult', $time_start , '0', json_encode( $req)  , $res, 'Social' ) ;
		print_r( ( $res)) ;
